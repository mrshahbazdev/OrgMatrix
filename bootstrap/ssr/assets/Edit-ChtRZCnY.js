import { withCtx, unref, createTextVNode, toDisplayString, createVNode, withModifiers, withDirectives, vModelText, vModelSelect, openBlock, createBlock, Fragment, renderList, vModelCheckbox, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  props: { organization: Object, role: Object, parentRoles: Array },
  setup(__props) {
    const { t } = useI18n();
    const props = __props;
    const form = useForm({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        header: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<h1 class="text-xl font-semibold text-gray-900"${_scopeId}>${ssrInterpolate(unref(t)("roles.edit"))}</h1>`);
          } else {
            return [
              createVNode("h1", { class: "text-xl font-semibold text-gray-900" }, toDisplayString(unref(t)("roles.edit")), 1)
            ];
          }
        }),
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<!--]--></select></div><label class="flex items-center gap-2"${_scopeId}><input${ssrIncludeBooleanAttr(Array.isArray(unref(form).is_active) ? ssrLooseContain(unref(form).is_active, null) : unref(form).is_active) ? " checked" : ""} type="checkbox" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"${_scopeId}><span class="text-sm text-gray-700"${_scopeId}>${ssrInterpolate(unref(t)("roles.is_active"))}</span></label><div class="flex gap-3 pt-4"${_scopeId}><button type="submit"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} class="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 transition disabled:opacity-50"${_scopeId}>${ssrInterpolate(unref(t)("common.save"))}</button>`);
                ...
                createVNode("div", { class: "rounded-2xl border border-gray-100 bg-white p-8 shadow-sm" }, [
                  createVNode("form", {
                    ...
                    createVNode("label", { class: "flex items-center gap-2" }, [
                      ...
                      createVNode("span", { class: "text-sm text-gray-700" }, toDisplayString(unref(t)("roles.is_active")), 1)
                    ]),
